...
class WebSkySZ:
    ...
    def y2uK_CMB(self,nu):
        h=6.62607004e-27; k=1.380622e-16; Tcmb=2.725
    ...
    def signal(self, nu, **kwargs):
        ...
        if np.isscalar(nu): nu = np.array([nu])
        
        m = np.zeros(12*self.nside**2)+1.0
        filename = utils.get_data_from_url(self.get_filename())            
        m = pysm.read_map(filename,nside=self.nside,field=0,
                             pixel_indices=self.pixel_indices,mpi_comm=self.mpi_comm)
        ...
        all_maps[:,0,:] = np.outer(pysm.convert_units("uK_CMB", "uK_RJ", nu)*szfac,m)
